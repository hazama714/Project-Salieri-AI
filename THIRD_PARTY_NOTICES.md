...
| Vosk library / Windows native runtime | REVIEW REQUIRED | Alpha Cephei / Vosk project | https://github.com/alphacep/vosk-api | Apache-2.0 | NO | Native runtime not redistributed | Preserve Apache-2.0 notice if redistributed | YES | Public source P/Invokes `libvosk`; exact runtime artifact/version未確定。 |
| Vosk Android library | 0.3.75 runtime identifier | Alpha Cephei / Vosk project | https://github.com/alphacep/vosk-api | Apache-2.0 | NO | AAR/native payload not redistributed | Preserve Apache-2.0 notice if redistributed | YES | Source imports `org.vosk` APIs。exact Maven artifactの再現確認はREVIEW REQUIRED。 |
| Vosk Japanese model | vosk-model-small-ja-0.22 | Alpha Cephei / model contributors | https://alphacephei.com/vosk/models | Apache-2.0 | NO | Model archive not redistributed | Preserve model license/notice if redistributed | YES | Runtime manifestはversion 0.22を要求。 |
| JNA | REVIEW REQUIRED | Java Native Access project | https://github.com/java-native-access/jna | LGPL-2.1-or-later OR Apache-2.0 | NO | Binary not redistributed | Distribution時に採用license optionのnoticeを確認 | YES, Android Vosk dependency | Resolved artifact/versionはREVIEW REQUIRED。 |
...
| R8 build tool artifact | 8.13.19 | Google | https://r8.googlesource.com/r8/+/refs/heads/main/LICENSE | BSD-3-Clause-style upstream license | NO | JAR not redistributed | Exact artifactに付属するnoticeを確認 | YES | `Assets/Plugins/Android/BuildTools/r8-8.13.19.jar`想定。artifact provenanceはREVIEW REQUIRED。 |
...
| Arduino AVR Core / Wire | User-installed; not pinned | Arduino | https://github.com/arduino/ArduinoCore-avr | Core files retain individual notices; Wire LGPL-2.1-or-later | NO | Boards Manager dependency | Preserve applicable source notices if redistributed | YES | Firmware build dependency。 |
